...
export default function AdminDashboard({ user, onLogout }) {
  ...
  const formatResultDate = (dateValue) => {
    if (!dateValue) {
      return 'Not finished';
    }

    const date = new Date(dateValue);
    return Number.isNaN(date.getTime()) ? 'Not finished' : date.toLocaleDateString();
  };
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-4xl font-bold text-gray-800 flex items-center gap-3">
              <Settings className="h-9 w-9 text-blue-600" aria-hidden="true" />
              Admin Dashboard
            </h1>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {activeTab === 'categories' && (
          <div className="space-y-6">
            ...
            <div className="card">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Tags className="h-6 w-6 text-blue-600" aria-hidden="true" />
                Categories
              </h2>
                      ...
                      {getFilteredResults().map((result, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-gray-600 text-sm">
                            {formatResultDate(result.finished_at)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
