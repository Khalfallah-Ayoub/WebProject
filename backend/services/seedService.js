...
const seedDatabase = async () => {
  ...
  try {
    ...
    // Create exam_sets and related tables
    try {
      await client.query("DROP TABLE IF EXISTS exam_set_questions CASCADE");
      await client.query("DROP TABLE IF EXISTS exam_sets CASCADE");
      ...
      console.log("[!] Table setup error:", tableError.message);
      throw tableError;
    }
...
const seedGroupsDatabase = async () => {
  ...
  try {
    ...
    // Create categories and questions for each programming language
    for (const [langKey, langData] of Object.entries(programmingLanguages)) {
      console.log(`[+] Adding category: ${langData.name}`);
    ...
    console.log(`[✓] Group: 1 | Categories: ${totalCategories} | Questions: ${totalQuestions}`);

    return {
      success: true,
      message: "Groups database seeded successfully!",
      groups: 1,
...
const seedCompleteDatabase = async () => {
  ...
  try {
    ...
    console.log("[+] Creating 'Programming Languages' group...");
    ...
    // ==================== CULTURE GROUP ====================
    console.log("[+] Creating 'ثقافة عامة' group...");
    ...
    return {
      ...
      categories: 11,
      totalQuestions: totalQuestions,
      totalAnswers: Math.round(totalQuestions * 3.5)
    };
...
module.exports = { seedDatabase, seedGroupsDatabase, seedCompleteDatabase };
